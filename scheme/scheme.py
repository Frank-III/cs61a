...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 4
        "*** YOUR CODE HERE ***"
        sch_list = expr.map(lambda x: scheme_eval(x, env))
        validate_procedure(sch_list.first)
        return scheme_apply(sch_list.first, sch_list.rest , env)
        # END PROBLEM 4
...
def eval_all(expressions, env):
    ...
    scheme_eval(expressions.first, env) # change this line
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target): 
        ...
        env.define(target.first, LambdaProcedure(body = expressions.rest, formals = formals, env = env) )
        return target.first
        # END PROBLEM 9
    else:
...
def do_lambda_form(expressions, env):
    ...
    # BEGIN PROBLEM 8
    "*** YOUR CODE HERE ***"
    return LambdaProcedure(body = expressions.rest, formals = formals, env = env)
    # END PROBLEM 8
