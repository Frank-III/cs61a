...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        def helper(curr_place, index):
            if curr_place == beehive:
                return None
            elif curr_place.bees and (index <= self.max_range and index >= self.min_range):
                return random_or_none(curr_place.bees)
            else:
                index += 1
                return helper(curr_place.entrance,index)  
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        for bee in all_bees:
            bee.reduce_armor(amount) if self.place else bee.reduce_armor(amount + self.damage)    
        # END Problem 5
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        # BEGIN Problem 13
        Ant.__init__(self,armor)
        if QueenAnt.true_ant:
            QueenAnt.true_ant = False
            self.true_ant = True
        # END Problem 13

    def remove_from(self,place):
        if isinstance(place.ant, ContainerAnt) and place.ant.contained_ant and not hasattr(place.ant.contained_ant, 'true_ant'):
            ContainerAnt.remove_from(self, place)
        elif not self.true_ant:
            Ant.remove_from(self, place)
    def action(self, gamestate):
        ...
        """
        # BEGIN Problem 13
        if not self.true_ant:
            Insect.reduce_armor(self,self.armor)
        else:
            ScubaThrower.action(self, gamestate)
            behind_place = self.place.exit
            while behind_place:
                if behind_place.ant and not behind_place.ant.buffed:
                    behind_place.ant.buffed = True
                    behind_place.ant.damage = behind_place.ant.damage * 2
                if isinstance(behind_place.ant, ContainerAnt) and behind_place.ant.contained_ant and not behind_place.ant.contained_ant.buffed:
                        behind_place.ant.contained_ant.buffed = True
                        behind_place.ant.contained_ant.damage = behind_place.ant.contained_ant.damage * 2
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 13
        Insect.reduce_armor(self,amount)
        if self.true_ant and not self.armor:
            bees_win()
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        gamestate -- The GameState, used to access game state information.
        """
        destination = self.place.exit if self.direction else self.place.entrance
...
def make_slow(action, bee):
    ...
    """
    # BEGIN Problem EC
    def real_make_slow(gamestate):
        if gamestate.time % 2 == 0:
            return action(gamestate)
    return real_make_slow
    # END Problem EC
...
def make_scare(action, bee):
    ...
    """
    # BEGIN Problem EC
    def real_make_scare(gamestate):
        bee.direction = False
        return action(gamestate)
    return real_make_scare
    # END Problem EC
...
def apply_effect(effect, bee, duration):
    ...
    def apply_effect_helper(gamestate):
        nonlocal duration
        if duration > 0:
            duration -= 1
            return effect(og_action,bee)(gamestate)
        else:
            bee.direction = True
            return og_action(gamestate)
    bee.action = apply_effect_helper                        
    # END Problem EC
